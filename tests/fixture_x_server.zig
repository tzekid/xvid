...
fn serve(io: std.Io, allocator: std.mem.Allocator, stream: std.Io.net.Stream, origin: []const u8, state: *State) !void {
    ...
    if (request.head.method == .GET and std.mem.endsWith(u8, request.head.target, "/encode-fail.mp4"))
        return imageResponse(&request, "video/mp4", "\x00\x00\x00\x18ftypisomfixture-mp4 encode-fail height=1080");
    if (request.head.method == .GET and std.mem.endsWith(u8, request.head.target, "/encode-stall.mp4"))
...
fn graphqlResult(allocator: std.mem.Allocator, origin: []const u8, state: *State, status_id: []const u8) ![]const u8 {
    ...
    if (std.mem.eql(u8, status_id, "2141")) {
        ...
        const media = try std.fmt.allocPrint(allocator, "{{\"id_str\":\"real-video\",\"type\":\"video\",\"original_info\":{{\"width\":320,\"height\":240}},\"video_info\":{{\"duration_millis\":1000,\"variants\":[{{\"content_type\":\"video/mp4\",\"url\":\"http://{s}/video/320x240/real.mp4\",\"width\":320,\"height\":240}},{{\"content_type\":\"video/mp4\",\"url\":\"http://{s}/video/160x120/alternate.mp4\",\"width\":160,\"height\":120}}]}}}}", .{ origin, origin });
        return tweet(allocator, origin, "Fixture User", "Generated local video", media);
    }
    if (std.mem.eql(u8, status_id, "2142")) return tweet(allocator, origin, "Fixture User", "Encoder failure", try video(allocator, origin, "encode-fail", "encode-fail"));
    if (std.mem.eql(u8, status_id, "2143")) return tweet(allocator, origin, "Fixture User", "Encoder cancellation", try video(allocator, origin, "encode-stall", "encode-stall"));
